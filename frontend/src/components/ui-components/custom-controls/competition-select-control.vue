...
export default {
  name: 'competition-select-control',
  data() {
    return {
      competitions: [],
...
<template>
  <div class="competitionSelect__wrapper">
    <div class="competitions__list">
      <div class="loadingFiller" v-if="is_loading">События загружаются...</div>
      <template v-else>
        <div class="competitions__list__item" v-for="competition in competitions" :key="competition.id">{{ competition.title }}</div>
      </template>
    </div>
  </div>
</template>
