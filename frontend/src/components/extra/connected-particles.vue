	...
	methods: {
		...
		setParticlePosition(particle) {
			...
			if (
				...
				particle.x >= this.properties.canvasWidth - this.properties.particleRadius
			) {
				particle.velocityX = -particle.velocityX
			}
			if (
				particle.y <= this.properties.particleRadius ||
				particle.y >= this.properties.canvasHeight - this.properties.particleRadius
			) {
			...
			this.particles.forEach((particle_1, p_idx) => {
				this.particles.forEach((particle_2, ip_idx) => {
					...
					if (distance <= this.properties.lineLength) {
						...
						const color =
							getComputedStyle(document.documentElement).getPropertyValue('--color-text-accent') +
					...
					if (distance < this.properties.lineLength / Math.log(distance ** 2)) {
						...
						const force =
							(this.properties.lineLength / 4 - distance) / this.properties.lineLength / 2
