...
export default {
	...
	methods: {
		...
		getAthlete(competitor_id) {
			return (
				this.competition.competitors.find(
					athlete => athlete.bib.toString() === competitor_id.toString()
				) || null
			)
		},
		getHeats(race_id) {
						...
						>
							<div class="resultsTable__tableValue" data-header-value="position">
								<div
									...
									:style="{
										backgroundColor: `var(${getSXHeatCompetitorColor(competitor_idx + 1)})`
									}"
								>
							...
							>
								...
								{{
									`${
										competitor && competitor.lastname ? competitor.lastname.toUpperCase() + ' ' : ''
									}${competitor.name || '-'}`
								}}
							</div>
