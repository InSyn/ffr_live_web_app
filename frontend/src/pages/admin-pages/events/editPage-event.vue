<template>
	<div class="editEventPage__wrapper">
		...
		<message-container :errors="errors" :messages="messages" />
	</div>
</template>
				...
				if (response.status === 200) {
					...
					setTimeout(() => {
						if (this.$route.name === 'editEventPage') {
							this.$router.push({ name: 'eventPage', params: { event_id: this.event_id } })
						}
					}, 1280)
				}
			} catch (err) {
				console.log(
					`Информация о событии не была обновлена: ${err.response?.data?.message || err.message}`
				)
				this.errors.push(
					`Информация о событии не была обновлена: ${err.response?.data?.message || err.message}`
				)
