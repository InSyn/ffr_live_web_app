...
export default {
	...
	methods: {
		...
		getRegisteredAthletesCount(group) {
			if (group) {
				return this.registeredEventApplications.reduce(
					(acc, item) => acc + item.athletes.filter(athlete => athlete.group === group).length,
					0
				)
			}
		...
		getApplicationAthletes(application, group) {
			return application.athletes
				.filter(athlete => !!athlete && athlete.group === group)
		...
		<div v-if="event" class="onlineRegistration__wrapper">
			...
			<div v-if="!selectedApplication" class="registrationInfo__full__wrapper">
				<div class="registrationStats__wrapper">
					<div class="registeredApplicationsOverall">
						Заявок: {{ registeredEventApplications.length }}
					</div>
					<div class="registeredAthletesOverall">
						...
						<button
							...
							type="button"
							@click.prevent="
								exportRegistrationApplicationAthletesToExcel(registeredEventApplications, true)
							"
						>
					...
					<div class="registeredEventGroups__wrapper">
						<div
							...
							:key="group"
							class="registeredEventGroups__item"
						>
				...
				<div class="registeredApplications__wrapper">
					...
					<div class="registeredApplications__body">
						<div class="registeredApplications__list">
							<div
								v-for="application in getRegisteredApplications"
								:key="application._id"
								class="registeredApplications__list__item"
							>
								...
								<button
									class="registeredApplications__list__item__action actionButton"
									type="button"
									@click.prevent="selectApplication(application)"
								>
									Открыть
								</button>
							</div>
			...
			<div v-else class="registrationInfo__application__wrapper">
				<div class="registrationInfo__application__header">
					<div class="registrationInfo__application__header__title">
						<button class="actionButton" type="button" @click.prevent="selectedApplication = null">
							Назад
						</button>
						<span>Заявка</span>
						<button
							class="downloadButton"
							type="button"
							@click.prevent="exportRegistrationApplicationAthletesToExcel(selectedApplication)"
						>
							Скачать список
					...
					<div class="registrationInfo__application__header__body">
						<div class="registrationInfo__application__region">
							{{ selectedApplication.region }}
						</div>
						<div class="registrationInfo__application__date">
							{{ formatDate(selectedApplication.created_at, { full: true }) }}
						</div>
					</div>
				</div>
				<div class="registrationInfo__application__athletesGroups">
					<div
						v-for="group in event.athletes_groups"
						:key="group"
						class="registrationInfo__application__athletesGroups__item"
					>
						<div class="registrationInfo__application__athletesGroups__item__title">
							Группа: {{ group }} {{ getRegisteredAthletesCount(group) }}
						</div>
				...
				<div class="registrationInfo__application__documents">
					...
					<div class="registrationInfo__application__documents__list">
						...
						>
							<file-icon class="document__icon" />
							{{ document.title }}
						</a>
					</div>
				</div>
