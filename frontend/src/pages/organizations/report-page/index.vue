...
<template>
	<div class="reportPage__wrapper">
		<loader-spinner v-if="(!organization || !report) && loading" />
		<div v-else-if="organization && report" class="report__wrapper">
			...
			<div class="reportPage__content" v-html="formattedContent" />
			<div v-if="report.files.length" class="reportPage__images__wrapper">
				<images-list :images="report.files" />
			</div>
		</div>
	</div>
</template>
...
.reportPage__wrapper {
	...
	box-shadow: var(--surface-shadow-l);
	...
	.report__wrapper {
		...
		.reportPage__content {
			...
			:deep(a) {
				...
				color: var(--color-interactive-brand-default) !important;
