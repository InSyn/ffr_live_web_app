	...
	<div class="seminarsControl__wrapper">
		<div class="seminarParticipants__select__wrapper">
			...
			<div class="addNewPerson__wrapper">
				...
				<v-btn class="addPerson__button" color="var(--color-text-primary)" text @click="addPerson">
					Добавить
				</v-btn>
			</div>
			...
			<div class="personal__list__wrapper">
				...
				<div class="personal__list">
					<div v-for="(person, idx) in personal" :key="idx" class="personal__item__wrapper">
						<span>{{ person.fullname }}&nbsp;-&nbsp;</span>
						<input v-model="person.role" type="text" />

						<span class="removeParticipant__button" @click="removeParticipant(idx)"> УДАЛИТЬ </span>
					</div>
				</div>
				<div class="personal__list__menu">
					<v-btn color="var(--color-text-accent)" text @click="updatePersonal"> Сохранить </v-btn>
				</div>
			</div>

			<message-container :errors="errors" />

			<v-btn class="closeButton" color="var(--color-text-error)" @click="closeControl">
				Закрыть
			</v-btn>
		</div>
	</div>
...
export default {
	name: 'SeminarParticipantsControl',
