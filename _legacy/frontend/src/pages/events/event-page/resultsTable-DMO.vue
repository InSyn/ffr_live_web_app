...
export default {
  ...
  methods: {
    ...
    isOddOrFinal(competition, selectedStageFilter) {
      ...
      const indexOfSelectedStage = competition.races.findIndex((race) => race.race_id === selectedStageFilter.target);

      if (!competition || !competition.races[indexOfSelectedStage]) return false;

      if (indexOfSelectedStage === competition.races.length - 1 || indexOfSelectedStage === competition.races.length - 2) return true;
    ...
    getCompetitorScore(competitor, raceFilter) {
      ...
      const stageScore = selectedRace.results.find((stageScore) => stageScore.competitor_id === competitor.local_id);
      ...
      <div class="raceFilters__group raceResultsFilters__wrapper">
        <button
          ...
          :class="[
            ...
            'raceResults',
            {
              isSelectedFilter: raceFilter.type === 'race-results' && raceFilter.target === race.race_id,
            },
          ]"
        >
          {{ race.title }}
          ...
          >
            <div v-for="(competitor, competitor_idx) in heat.competitors" :key="competitor_idx" class="resultsSheet__competitorResult">
              <div class="resultsTable__tableValue" data-header-value="position">
                <div class="sx-color-wrapper" :style="{ backgroundColor: `var(${getDMOHeatCompetitorColor(competitor_idx + 1)})` }">
              ...
              <div class="resultsTable__tableValue" data-header-value="name" @click="navigateToAthletePage(competitor.ffr_id)">
                <div v-if="competitor.region || competitor.country" class="athleteFlags__wrapper">
                  <country-flag
                    v-for="(athleteRegion, rIdx) in competitor.region ? competitor.region.split(',') : competitor.country ? [competitor.country] : []"
                    :key="rIdx"
