...
import CupStagesControl from '@/pages/admin-pages/events/cupStages-control.vue';
import CompetitionSelectControl from '@/components/ui-components/custom-controls/competition-select-control.vue';

export default {
  name: 'form-cupEvent',
  components: { CompetitionSelectControl, CupStagesControl, CustomCheckbox, DocumentsSelectControl, CountrySelectControl, CompetitionImageFillerIcon },
  props: {
    event: Object,
    eventImages: Object,
    action: String,
...
<template>
  <form enctype="multipart/form-data" @submit.prevent="submitForm">
    <div class="formHeader">
      <span v-if="action === 'create'">Новое кубковое событие</span>
      <span v-else>Обновление данных кубкового события</span>
    ...
    <div class="formBody">
      ...
      <div v-for="(_, field_key) in event" :key="field_key" class="formGroup">
        ...
        ></documents-select-control>
        <custom-checkbox v-else-if="field_key === 'is_public'" :value="event[field_key]" @input="setFieldValue(event, field_key, $event)"></custom-checkbox>
        <input v-else v-model="event[field_key]" :id="field_key" class="formControl" :type="getInputType(field_key)" :name="field_key" />
      </div>
    </div>

    <cup-stages-control class="cupStages__control" :cup-event="event"></cup-stages-control>
    <competition-select-control></competition-select-control>
